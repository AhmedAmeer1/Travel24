...
class Customer_model extends CI_Model {
	...
	function get_customer($customer_id = '',$view_all = 0){
		...
		$cond .= (!empty($customer_id))?" AND id = '$customer_id[customer_id]'":"";
		$result = $this->db->query("SELECT * FROM user WHERE $cond");
		$val=$result->result();
		//print_r($val);exit;
		if(empty($val)){
			return;
	...
	function changeStatus($customer_id = '', $status = '0'){
		...
		$status = $this->db->update('user',array('status'=>$status), array('id'=>$customer_id));
		return $status;
	}
}
?>